...
def benchmark(source_dir, results_file):
    ...
    print("-" * 80)
    print(
        f"{'Method':<15} {'Level':<5} {'Size':<12} {'Ratio':<8} {'Comp (s)':<10} {'Ext (s)':<10}"
    )
    print("-" * 80)
    ...
    for config in configs:
        ...
        for level in config["levels"]:
            ...
            comp_time = run_cmd(
                config["compress"].format(out=out_file, src=source_dir, l=level)
            )
            ...
            print(
                f"{config['name']:<15} {level:<5} {format_bytes(size):<12} {ratio:>6.2f}x {comp_time:>10.2f} {extract_time:>10.2f}"
            )
    ...
    with open(results_file, "w") as f:
        ...
        for r in sorted(results, key=lambda x: x["Size_B"]):
            f.write(
                f"| {r['Method']} | {r['Level']} | {r['Size']} | {r['Ratio']} | {r['Comp_s']} | {r['Ext_s']} |\n"
            )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Advanced Benchmark for compression algorithms."
    )
    parser.add_argument(
        "source",
        nargs="?",
