...
class TestOptionalDependencies(unittest.TestCase):
    ...
    def test_download_file_rejects_non_http_runtime_archive_url(self):
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            with patch.object(
                deps_runtime_command.urllib.request, "urlopen"
            ) as urlopen:
                with self.assertRaisesRegex(
                    deps_runtime_command.WGSExtractError,
                    "Unsupported runtime archive URL",
                ):
                    deps_runtime_command._download_file(
                        "file:///tmp/runtime.zip", destination
                    )
