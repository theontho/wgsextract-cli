...
class TestMicroarrayRealData(unittest.TestCase):
    ...
    def run_samtools_mpileup(self, chrom, pos):
        ...
        cmd = ["samtools", "mpileup", "-r", region, "-f", self.ref_fasta, self.isolated_input]
        ...
        while i < len(bases_str):
            b = bases_str[i]
            if b in ".,": called_bases.append(ref_base)
            elif b in "ACGTNacgt": called_bases.append(b.upper())
            elif b == "^": i += 1
            elif b in "+-":
                num_str = ""
                i += 1
    ...
    def test_microarray_multi_vendor(self):
        ...
        args = [
            'wgsextract-cli', '--outdir', self.test_dir, 
            '--ref', REF_PATH, '--input', self.isolated_input, 
            'microarray', '--formats', ",".join(formats_to_test)
        ]
        ...
        for fmt_key, filename in vendor_files.items():
            zip_path = os.path.join(self.test_dir, filename.replace(".txt", ".zip").replace(".csv", ".zip"))
            self.assertTrue(os.path.exists(zip_path), f"Output ZIP missing for {fmt_key}")
            
            with zipfile.ZipFile(zip_path, 'r') as zip_ref:
                zip_ref.extractall(self.test_dir)
            
            out_path = os.path.join(self.test_dir, filename)
            self.assertTrue(os.path.exists(out_path), f"Extracted file missing for {fmt_key}")
            ...
            for line in data_lines:
                ...
                if snp_id in verified_snps:
                    ...
                    if source_geno:
                        print(f"[{fmt_key}] SNP {snp_id}: Output={genotype}, Source={source_geno}")
                        for b in genotype:
