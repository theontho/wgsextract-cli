...
class TestInfoCommand(unittest.TestCase):
    ...
    def test_info_fast_mode(self, mock_exists, mock_stats, mock_sorted, mock_md5, mock_verify):
    ...
    def test_info_detailed_mode(self, mock_open, mock_getsize, mock_exists, mock_idxstats, mock_body, mock_stats, mock_sorted, mock_md5, mock_verify):
        ...
        mock_body.return_value = (1000, 150.0, 2.0, 300.0, 50.0, True, "A00910:1:ABCDEFGHI:1:1:1:1")
    ...
    @patch('wgsextract_cli.commands.info.os.path.getsize')
    @patch('wgsextract_cli.commands.info.open', create=True)
    def test_info_csv_mode(self, mock_open, mock_getsize, mock_exists, mock_idxstats, mock_body, mock_stats, mock_sorted, mock_md5, mock_verify):
        mock_md5.return_value = "bd894134bddba260df88a90123a2ee9c"
        mock_sorted.return_value = True
        mock_stats.return_value = (50.0, True)
        mock_exists.return_value = True
        ...
        mock_idxstats.return_value = ([{"name": "1", "length": 100, "mapped": 10, "unmapped": 0}], 100, 10, 0)
        
        test_args = ['wgsextract-cli', '--input', 'sample.bam', 'info', '--detailed', '--csv']
        ...
        output = f.getvalue()
        self.assertIn("Seq Name,Model Len,Model N Len,# Segs Map,Map Gbases,Map ARD,Breadth Coverage", output)
