...
class TestE2EBase(unittest.TestCase):
    ...
    def tearDownClass(cls):
        ...
        print("-" * 105)
        print(f"TOTAL REAL DATA ({cls.MODE}): {passed}/{len(cls.results)} passed. Total Time: {total_duration:.2f}s")
        print("="*105)
        ...
        try:
            ...
            with patch.object(sys, 'argv', full_args):
                ...
                if duration < min_duration:
                    print(f"Error: {name} finished too fast ({duration:.2f}s < {min_duration}s)")
                    success = False
                else:
                    success = True
    ...
    def test_06_bam_sort_chrm(self): 
        ...
        self.run_real("06 bam sort (chrM)", ['bam', 'sort', '--region', 'chrM', '--input', INPUT_PATH, '--ref', REF_PATH], 'GenSortedBAM', min_duration=min_d)
    
    def test_07_bam_tocram_chrm(self): 
        min_d = 60 if self.REGION is None else 0
        self.run_real("07 bam to-cram (chrM)", ['bam', 'to-cram', '--region', 'chrM', '--input', INPUT_PATH, '--ref', REF_PATH], 'BAMtoCRAM', min_duration=min_d)
    ...
    def test_13_vcf_snp_chrm(self): 
        ...
        self.run_real("13 vcf snp (chrM)", ['vcf', 'snp', '--region', 'chrM', '--input', INPUT_PATH, '--ref', REF_PATH], 'ButtonSNPVCF', min_duration=min_d)
    ...
    def test_35_vcf_cnv_chrm(self):
        min_d = 300 if self.REGION is None else 0
        self.run_real("35 vcf cnv (chrM)", ['vcf', 'cnv', '--input', INPUT_PATH, '--ref', REF_PATH], 'ButtonBAMStats2', min_duration=min_d)
