...
def print_warning(action_key, app_name=None, size_gb=None, final_gb=None, threads=None, file_size=None, is_cram=False):
    ...
    if action_key in EXPECTED_TIME:
        ...
        if threads and action_key in ['ButtonAlignBAM', 'ButtonUnalignBAM', 'GenSortedBAM', 'ButtonSNPVCF', 'ButtonInDelVCF']:
            try:
                t = int(threads)
                if t > 1:
        ...
        logging.warning(f"!!! WARNING: {MESSAGES['ExpectedWait'].format(time=time_str)} !!!")
    ...
    if action_key == 'infoFreeSpace':
        ...
        if app_name and size_gb is not None and final_gb is not None:
            msg = MESSAGES['infoFreeSpace'].format(app=app_name, size=size_gb, final=final_gb)
        ...
        try:
            ...
            msg = MESSAGES['RealignBAMTimeWarnMesg'].format(time=f"{estimated_hours:.1f}")
            logging.warning(f"!!! {msg} !!!")
        except:
            pass
