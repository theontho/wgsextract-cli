    ...
    def setup_ui(self) -> None:
        ...
        if key in ["gen", "ext", "anc", "vcf", "fastq"]:
            ...
            self.input_entry = self.create_file_selector(
                self,
                "Input:",
                variable=self.main_app.input_path_var,
                on_change=cb,
            )
        ...
        if key == "gen":
            ...
            self.vcf_vep_cache = self.create_read_only_entry(
                self, "VEP Cache:", self.main_app.vep_cache_var
            )
