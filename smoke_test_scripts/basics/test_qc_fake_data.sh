...
if uv run wgsextract qc fake-data \
    ...
    --coverage 0.1 \
    --seed 123 \
    --ref "$OUTDIR/hg38" && [ -f "$OUTDIR/hg38/fake.bam" ] && [ -f "$OUTDIR/hg38/fake.vcf.gz" ]; then
...
if uv run wgsextract qc fake-data \
    ...
    --build hg19 \
    --type cram \
    --coverage 0.05 && [ -f "$OUTDIR/hg19/fake.cram" ]; then
...
if uv run wgsextract qc fake-data \
    --outdir "$OUTDIR/fastq" \
    --type fastq \
    --coverage 0.01 && [ -f "$OUTDIR/fastq/fake_R1.fastq.gz" ]; then
