...
uv run wgsextract ref download \
    --url "http://localhost:8080/$(basename $REF_PATH)" \
    --out "$OUTDIR/downloaded.fa"
...
if [ $DOWNLOAD_EXIT -eq 0 ] && [ -f "$OUTDIR/downloaded.fa" ]; then
...
uv run wgsextract ref index \
    --ref "$OUTDIR/downloaded.fa"

if [ $? -eq 0 ] && [ -f "$OUTDIR/downloaded.fa.fai" ]; then
...
uv run wgsextract ref count-ns \
    --ref "$OUTDIR/downloaded.fa"
