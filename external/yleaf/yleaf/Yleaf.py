...
def main_vcf_split(
    ...
    if len(chry) == 0:
        ...
        new_position_bed_file = base_out_folder / (vcf_file.name.replace(".vcf.gz", "_temp_position_bed.bed"))
        with open(position_bed_file, "r") as f:
            with open(new_position_bed_file, "w") as f2:
                for line in f:
                    line = line.replace("chrY", chry[0])
                    if len(line.split()) >= 3 and "-1" not in line: f2.write(line)
...
def predict_haplogroup(
    ...
    if use_old:
        ...
        cmd = "{} {} -i {} -o {}".format(sys.executable, script, path_file, output)
        call_command(cmd)
    else:
